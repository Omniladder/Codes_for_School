...
1.  Obtain the tally table (including Count and Proportion) for the variable **Activity**.
    ...
    ```{r}
    ActCount <- data2 %>% group_by(Activity) %>%
      summarise(Count=n())

    ActProportion <- round(ActCount$Count/sum(ActCount$Count),2)

    ActStat <- cbind(ActCount,ActProportion)
    flextable(ActStat)
    ```
...
2.  Plot the Pie Chart for Activity. (When copy and paste the above code, don't forget to add another color (Sex has two values but Activity has three))
    ...
    ```{r}
    ggplot(ActStat,aes(x="",y=ActProportion,fill=Activity))+
      geom_bar(stat="identity")+
      coord_polar("y",start=0)+
      theme_void()+
      geom_text(aes(label=paste("Count=",Count,"  Prop=", scales::percent(ActProportion))),position=position_stack(vjust=0.5))+
      scale_fill_manual(values=c("lightblue","darkblue","blue"))
    ```
...
Yes their is as Males are much more likely to smoke than females we can see as their is a 23% chance that fa woman smokes but a 71% if male
