...
def get_pairs(word_freq_dict):
    ...
    for word, freq in word_freq_dict.items():
        ...
        tokens = word.split(' ')
        for tokenIndex in range(len(tokens) - 1):
            pairs[(tokens[tokenIndex], tokens[tokenIndex + 1])] += freq
        # your code ends here
    return pairs
...
def exract_bpe_subwords(text, steps):
    ...
    for i in tqdm(range(steps)):
        ...
        most_frequent_pair = get_most_frequent_pair(token_pairs)

        # merge the token pair with highest frequency
        word_freq_dict = merge_byte_pairs(most_frequent_pair, word_freq_dict)
