...
class NPLMFirstBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        linear_embedding = self.linear(embedding)
        tanh_embedding = torch.tanh(linear_embedding)

        # apply layer normalization
        normal_embedding = self.layer_norm(tanh_embedding)

        # apply dropout
        final_embeds = self.dropout(normal_embedding)
        # your code ends here
...
class NPLMBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        linear_embedding = self.linear(inputs)
        tanh_embedding = torch.tanh(linear_embedding)

        # add residual connection
        tanh_embedding = tanh_embedding + self.hidden_dim 

        # apply layer normalization
        normal_embedding = self.layer_norm(tanh_embedding)

        # apply dropout
        final_inputs = self.dropout(normal_embedding)
...
class NPLMFinalBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        log_probs = F.log_softmax(linear_embedding, dim=-1)
