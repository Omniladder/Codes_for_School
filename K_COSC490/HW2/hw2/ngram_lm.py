...
from collections import (
    Counter,
    defaultdict,
)  # we will using "Counter" data structure for counting word co-occurences
...
def load_data():
    ...
    dev_dataset = load_dataset(
        path="wikitext", name="wikitext-103-raw-v1", split="validation"
    )
...
def create_ngrams(data, n, splitter, tokenizer):
    ...
    for paragraph in tqdm(data["text"]):
        ...
        for sentence in splitter.split(paragraph):
            ...
            for idx, _ in enumerate(tokens):
                ...
                next_word_candidates[tuple(tokens[idx : idx + n - 1])].add(
                    tokens[idx + n - 1]
                )
                # Your code ends here
    ...
    for context, next_words in sorted_next_word_candidates.items():
        ...
        for nw in next_words:
            ...
            scores.append(ngrams[context + tuple([nw])] / ngram_context[context])
            # Your code ends here
...
def plot_next_word_prob(word_scores, word_candidates, context, top=10, save_path=None):
    ...
    (word_probs, words) = zip(
        *sorted(zip(word_scores[context], word_candidates[context]), reverse=True)[:top]
    )

    plt.figure(figsize=(10, 6))
    plt.barh(words, word_probs, color="skyblue")
    plt.xlabel("Probability")
    plt.title(f"Top {top} Next Word Probabilities for Context: {context}")
...
def run_ngram():
    ...
    word_pred, next_word_scores, next_word_candidates = create_ngrams(
        train_data[:100000], 3, splitter, tokenizer
    )
    ...
    context2 = ("the", "news")
    print(
        f"{'-' * 10} plot the top 10 next word probabilities after {context1} {'-' * 10}"
    )
    plot_next_word_prob(
        next_word_scores,
        ...
        context1,
        top=10,
        save_path="ngram_context1.png",
    )
    print(
        f"{'-' * 10} plot the top 10 next word probabilities after {context2} {'-' * 10}"
    )
    plot_next_word_prob(
        next_word_scores,
        next_word_candidates,
