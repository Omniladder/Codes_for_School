...
class SentimentClassifier(nn.Module):
    ...
    def softmax(logits):
        ...
        return nn.Softmax(dim=1)(logits - torch.max(logits, dim=1, keepdim=True).values)
        # your code ends here
    ...
    def gradient_loss(self, inp, logits, labels):
        ...
        soft_max_logits = self.softmax(logits)
        grads_weights = torch.matmul(inp.T, (soft_max_logits - labels)) / bsz
        grads_bias = (soft_max_logits - labels) / bsz
        loss = -torch.mean(labels * torch.log(soft_max_logits))
...
def test_softmax():
    ...
    assert torch.allclose(
        SentimentClassifier.softmax(test_inp1),
        torch.FloatTensor([[0.26894143, 0.73105860], [0.26894143, 0.73105860]]),
    )
    assert torch.allclose(
        SentimentClassifier.softmax(test_inp2),
...
def test_gradient_loss(model: SentimentClassifier):
    ...
    assert torch.allclose(
        gw1,
        torch.FloatTensor([[0.8829, 1.3623, 1.8418], [-0.8829, -1.3623, -1.8418]]),
        atol=1e-4,
    )
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(train_dataloader):
            ...
            with torch.no_grad():
                ...
                model.linear.weight.data = model.linear.weight.data - (
                    learning_rate * grads_weights
                )
                model.linear.bias.data = model.linear.bias.data - (
                    learning_rate * grads_bias
                )
                # your code ends here
