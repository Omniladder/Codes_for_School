...
\begin{document}
    ...
    \begin{itemize}
        ...
        \solution{      
            ...
            The eigenvalue of $\textbf{B}$ are 3, 1, and 0 
            Thus,  \[
                \textbf{D} = 
                \begin{bmatrix}
                    0 & 0 & 0 \\
    ...
    \begin{enumerate}
        ...
        \solution{$P\left(B\right)=.91\overline{6}$}
    \end{enumerate}
    ...
    \begin{enumerate}
        ...
        \solution{
            $\\.5 \times .3 + .5 \times .9 = .6$ thus we know there is a 
            50\%  chance of a choosing a coin and it landing on heads given 
            this with 3 flips we have $3 \times .6 = 1.8$ which is $\mu$ 
            and the expected value
        }
        \item What is $\var{X}$?
        \solution{
            \\Using the formula for the probability of probability distribution we know 
            $\sum_{i=0}^n \left( P(x_i) \cdot (x_i - \mu)^2\right)$ We have $2^3$ permutations of 
            head flips and $\binom{x_i}{3}$ permutations of each number of heads $x_i$ knowing this
            we have the following formulas $\frac{1}{8} \left( 0-1.8 \right)^2 + \frac{3}{8} 
            \left( 1-1.8 \right)^2  + \frac{3}{8} \left( 2-1.8 \right)^2 + \frac{1}{8} 
            \left( 3-1.8 \right)^2 = .84 \\ \therefore \var{X} = .84$
        }
        \item Based on the number of heads we get, we earn $Y = \frac{1}{2 + X}$ dollars. 
        What is $\expec{Y}$?
        \solution{\\Plugging in the answer from problem 1 we know $E(X) = .6$ thus, $E(Y) = \frac{1}{2 + .6} = .384\dots$}
    \end{enumerate}
    ...
    \solution{
        \\ The reason why this isnt a contradiction is because as stated the first statements 
        assumes that all variables are independent while the second example using 2 X's is
        obviously perfectly coorelated with eachother and thus, the top formula doesnt apply
        \\$\therefore$ this is not a contradiction
    }
    ...
    \begin{enumerate}
        ...
        \solution{$1 - \left( e^{-x} \right) p\left(x\right) $}
    \end{enumerate}
    ...
    \begin{enumerate}
        ...
        \solution{$
            \mathcal{O}\left({\log\left(n\right)}\right)
        $}
    ...
    \noindent\solution{The Dev loss is generally consistently higher than the training loss implting that there is a degree of overfitting. we also see that at some point such as epoch 2 and 15 are noticeable in there increase while the training data is decreased implying overfitting got worse in those two examples}
    ...
    \noindent\solution{We can see that wordtovec is consistently the best embedding and generally gets better over time this is likely because of the seemingly larger nature of the embedding dataset and the more professional nature of google news over twitter.}
