...
# alternative is either "two.sided" or "greater" or "less"
#conf.level = 1- alpha
t.test(x=x, mu=42, alternative = "greater", conf.level = 1-.05 )
```

```         
1. Null: the mean hegiht of the first grader in that region is equal to 42 inches

  Alternative: the mean height of the first graders in that region is greater than 42 inches
  
2. H0: mu = 42 vs HA: mu>42

3. Alpha = 0..05, it is the prb of making type I error, it is also the prob standard we used to define and unusual event

4. since the sample size n=75 > 30 It is a large sample since we are not given the population standard deviation sigma. We should use 1-sample t.

5. If the computed p=pvalue < alpha, we have sufficient evidence to reject H0 and to support HA. If the computed p-value >= alpha we DO NOT have sufficient evidence to support HA

p-value = the probability of getting such a same (ot more extreme sample) if we assume H0 is true

6. Using R to compute the p-value = .02718 < 0.05 = alphsa. This sample of 73 student's height provide sufficent evidence to reject H0
...
8. The above decision is possible top be an error

Type I error: reject H0 when it is true (p - value < alpha)
Type II error: failed to reject H0 when it is not true

since our decision is reject H0, it is possible to be an type I error, with the probability, with the probability alpha = 0.05.
```
...
t.test(x=y, mu=43, alternative = "greater", conf.level = 1-.05 )
```

```         
1. Null: the mean weight of the first grader in that region is equal to 43 inches

  Alternative: the mean height of the first graders in that region is greater than 43 pounds
  
2. H0: mu = 43 vs HA: mu>43

3. Alpha = 0.05, it is the probability of making type I error, it is also the prob standard we used to define and unusual event
...
6. Using R to compute the p-value = 0.05376 > 0.05 = alphsa. This sample of 73 student's height provide sufficent evidence to reject HA

7. It means, the sample provide sufficient evidence to support the statement: "the mean height of the first graders in that region is greater than 43 pounds."
...
since our decision is reject HA, it is possible to be an type II error, with the probability, with the probability alpha = 0.05.
```
...
```         
due too a high p - value we can tell that H0 shouldn't be rejected and thus a 1 sample t test is still fine to continue
```

```{r}
expontetialRandom<-rexp(25,12)

ad.test(expontetialRandom)
qqPlot(expontetialRandom)
```

```         
The P value heres varies a bit occasionly just going over the mark but in its current example is dramatically under wanted p value thus H0 is rejected and shouldn't be continued
```

```{r}
caunchyRandom<-rcauchy(25, location = 0, scale = 12)

ad.test(caunchyRandom)
qqPlot(caunchyRandom)
```

```         
once again P-value is low and thus shouldnt be considered normal and thus rejected
```
...
```{r}
pennyData <- read.csv("Banking.csv")

boxplot(pennyData$X1991)

ad.test(pennyData$X1991)

wilcox.test(pennyData$X1991)
```

```         
1. Null: the median weight of a penny is at least 450 grams

  Alternative: the median weight is of a penny is less than 450 grams
  
2. H0: xˉ >= 450 vs HA: xˉ < 450

3. Alpha = 0..05, it is the prb of making type I error, it is also the prob standard we used to define and unusual event

4. since the sample size n=10 < 30 It is not a large sample since it is not approximately normal as the p value > .05 and the data is more or less symmetric we will use the wilcoxon test.

5. If the computed p=pvalue < alpha, we have sufficient evidence to reject H0 and to support HA. If the computed p-value >= alpha we DO NOT have sufficient evidence to support HA

p-value = the probability of getting such a same (ot more extreme sample) if we assume H0 is true

6. Using R to compute the p-value = .0.001953 < 0.05 = alphsa. This sample of 10 pennies weight provide sufficent evidence to reject H0

7. It means, the sample provide sufficient evidence to support the statement: "The median weight of a penny is less than 450 grams."
...
```{r}
ad.test(pennyData$X1990)
boxplot(pennyData$X1990)

SIGN.test(pennyData$X1990)
```

```         
the p value here is dramatically below the needed .05 theirfore this is a normal distibution and has a finite mean and standard deviation althought the standrard deviaiton is unknown theirfore the t test will be used

1. Null: the median weight of a penny is at least 350 grams

  Alternative: the median weight is of a penny is less than 350 grams
  
2. H0: xˉ >= 350 vs HA: xˉ < 350

3. Alpha = 0..05, it is the prb of making type I error, it is also the prob standard we used to define and unusual event

4. since the sample size n=10 < 30 It is not a large sample since it is not approximately normal as the p value 0.002151 < .05 and seems to have a rightward skew we will use the sign test.
